...
export default function NewJobScreen() {
  ...
  const [createJobData, setCreateJobData] =
    useState<CreateJobSchemaInterface>();
  ...
  const {
    ...
    formState: { errors },
    getValues,
  } = useForm<CreateJobSchemaInterface>({
      ...
      if (createJobData !== undefined) {
        try {
          ...
          const newJobData = await createJob(token, {
            ...
            modality: createJobData.modality as
              | "presential"
              | "hybrid"
              | "remote",
        ...
        <form className={styles.new_job_form} onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className={styles.description_row}>
            <Input
              ...
              name="description"
              maxLength={500}
              placeholder="Descreva a sua vaga."
            />
          </div>
